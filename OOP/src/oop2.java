...
class array1{
	...
	public array1() {
		...
		String ar2[] = newdata.split(",");	//문자배열, 해당 저장된 문자열을 split으로 구분하여 배열로 임시전환, (split는 절대 숫자 안됨)
		int ar2_ea = ar2.length;
		int oridata[] = new int[ar2_ea];	//숫자배열, 문자열 갯수만큼 배열 생성자 값을 추가
		
		int ww = 0;
		while(ww<ar2_ea) {
			oridata[ww] = Integer.parseInt(ar2[ww]);	//문자를 숫자로 변환하여 신규 배열 생성자에 값을 저장
			ww++;
		}
